...
[Deep Bellman Hedging](https://arxiv.org/abs/2207.00932v3) (revision 2.03, April
...
| Equation (20), vanilla Deep Hedging operator `T̃`; §5.1 discounted cash-invariance | `VanillaModel.bellman`, `VanillaModel.bellman_cash` (`VanillaBellman.lean`) |
| Theorem 2 (`th:convergence_dh`), existence and uniqueness | `VanillaModel.existsUnique_value` |
...
| §4 proposition, finite statistical arbitrage | `utility_gains_le_of_finite_arbitrage` (`StatArb.lean`) |
...
- The paper's "rewards are finite" hypothesis (equation (6)) is pointwise in the
...
- Theorem 2's second claim, that the solution equals the vanilla Deep Hedging cash
  value function for the entropic or expectation utility, is not formalised.
...
- The multi-step operator `T_n` of the paper's remark on multiple time steps, and its
  coincidence with the iterate `T^[n]` under the entropic utility, are not formalised:
  `HedgingModel` carries one utility per state and no product structure on outcomes.
