...
from hedging_gym.environment.paper_benchmarks import buehler_heston, szehr_market
...
def test_scheme_roundtrip_and_omitted_scheme_uses_current_default():
    for market_class in (HestonConfig, BatesConfig):
        ...
        values = asdict(explicit)
        values["market"].pop("scheme")
        restored = config_from_dict(values)
        assert restored == explicit
        assert market_from_dict(values["market"]) == restored.market
        assert "scheme" not in values["market"]
