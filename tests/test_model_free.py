...
from hedging_gym.baselines._shared.d4pg import (
    _accumulated_cost,
    action_gradient_loss,
    collect_episodes,
    quantile_huber_loss,
)
from hedging_gym.baselines.deep_hedging import DirectDHPolicy
from hedging_gym.baselines.exdrl import ParetoCritic, gpd_expected_excess, gpd_nll
from hedging_gym.baselines.exdrl import train as train_exdrl
from hedging_gym.baselines.qr_d4pg import QuantileCritic
from hedging_gym.baselines.qr_d4pg import train as train_qr_d4pg
from hedging_gym.environment.benchmark import benchmark_config
from hedging_gym.environment.config import PortfolioConfig, TimeGrid
...
def test_exdrl_tail_is_used_for_targets_and_actor_gradients():
    critic = ParetoCritic(3, 2, hidden=(8,), quantiles=32, tail_threshold=.8).double()
...
def test_ru_threshold_gradient_keeps_money_units_after_critic_scaling():
    critic = QuantileCritic(1, 1, hidden=(), quantiles=4).double()
    with torch.no_grad():
...
def test_model_free_updates_detached_environment_and_common_evaluation(method, stock_only):
    ...
    policy, metadata = {'hull_rl': train_qr_d4pg, 'exdrl': train_exdrl}[method](bank, seed=7, updates=3, batch_size=8,
...
def test_model_free_checkpoint_resume_matches_uninterrupted_training(tmp_path, method):
    ...
    full, _ = {'hull_rl': train_qr_d4pg, 'exdrl': train_exdrl}[method](bank, updates=4, checkpoint_path=full_path, **options)
    {'hull_rl': train_qr_d4pg, 'exdrl': train_exdrl}[method](bank, updates=2, checkpoint_path=split_path, **options)
    resumed, _ = {'hull_rl': train_qr_d4pg, 'exdrl': train_exdrl}[method](bank, updates=4, checkpoint_path=split_path,
...
def test_critic_warmup_freezes_actor_but_updates_critic(tmp_path):
    ...
    torch.manual_seed(7)
    initial = DirectDHPolicy(config, hidden=(8,)).double()
    policy, metadata = train_exdrl(bank, seed=7, updates=2, batch_size=8,
...
def test_explicit_frozen_warmup_extension_matches_declared_longer_run(tmp_path):
    ...
    full, _ = train_exdrl(bank, updates=4, actor_warmup_updates=3, **options)
    path = tmp_path/"frozen.pt"
    train_exdrl(bank, updates=2, actor_warmup_updates=2, checkpoint_path=path, **options)
    with pytest.raises(ValueError, match="saved training options"):
        train_exdrl(bank, updates=4, actor_warmup_updates=3, resume_from=path, **options)
    resumed, _ = train_exdrl(bank, updates=4, actor_warmup_updates=3,
        resume_from=path, checkpoint_path=path, extend_frozen_warmup=True, **options)
    for actual, expected in zip(resumed.parameters(), full.parameters()):
        torch.testing.assert_close(actual, expected, rtol=0, atol=0)
    with pytest.raises(ValueError, match="never updated"):
        train_exdrl(bank, updates=5, actor_warmup_updates=5,
