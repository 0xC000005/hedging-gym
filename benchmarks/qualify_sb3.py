...
from hedging_gym.baselines.ppo import build_ppo, ppo_controller
...
def main():
    ...
    model = build_ppo(env, seed=args.seed, device=args.device, epochs=args.epochs)
    progress = Progress(total, steps_per_rollout)
    records = []
    for phase in range(args.phases+1):
        _, sample = evaluate_controller(ppo_controller(model, deterministic=False), calibration,
        ...
        for deterministic in (True, False):
            ...
            metrics, tape = evaluate_controller(ppo_controller(model, deterministic=deterministic),
    ...
    _, loaded_tape = evaluate_controller(ppo_controller(reloaded), development,
