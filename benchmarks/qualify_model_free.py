...
from hedging_gym.baselines._shared.d4pg import collect_episodes
from hedging_gym.baselines.deep_hedging import DirectDHPolicy
from hedging_gym.baselines.exdrl import ParetoCritic
...
def diagnose(checkpoint, bank, *, device="cpu", probe_paths=2048):
    ...
    critic_class = ParetoCritic if saved["method"] == "exdrl" else QuantileCritic
    critic_options = dict(quantiles=options["quantiles"])
    if saved["method"] == "exdrl":
        critic_options["tail_threshold"] = options["tail_threshold"]
    critic = critic_class(actor.feature_dim, config.n_assets, options["hidden"],
                          **critic_options).to(device=device, dtype=bank.spot.dtype)
...
def main():
    ...
    while start < args.updates:
        ...
        policy, metadata = {'hull_rl': train_qr_d4pg, 'exdrl': train_exdrl}[args.method](training, **options,
