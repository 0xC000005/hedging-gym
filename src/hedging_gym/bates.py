...
def bates_transition(spot, variance, shocks=None, config=None, *, generator=None):
    ...
    """
    from .finance import BatesConfig, heston_transition
    ...
    continuous, next_variance = heston_transition(
        spot, variance, None if shocks is None else shocks[..., :3], config, generator=generator)
    ...
    if shocks is None:
        rate = torch.full_like(continuous, config.jump_intensity * config.dt)
        ...
        normal = torch.randn(continuous.shape, dtype=continuous.dtype, device=continuous.device,
                             generator=generator)
    log_jump = (-config.jump_intensity * jump_compensator(config) * config.dt
...
def bates_call_price(spot, variance, maturity, strike, config=None):
    ...
    from .finance import BatesConfig, heston_call_price
