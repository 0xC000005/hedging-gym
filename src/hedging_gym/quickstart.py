"""ADAPTATION / usage example: complete episodes and a market-only A -> B -> A.
...
def run(device="cpu", paths=64):
    ...
    basic = benchmark_config()
    operational = operational_config(basic, "operational_fixed",
    ...
    print(f"ADAPTATION / USAGE EXAMPLE; device={device}; CPU threads=4; "
    ...
    print("Actions are target stock/call holdings, not trade sizes. Reward is zero "
          ...
          "ES95 is the pooled upper tail of losses, not the per-step reward.", flush=True)
    ...
    for index, (name, config) in enumerate(stages):
        ...
        try:
            ...
            print(f"DONE {name}: mean P&L={float(reward.mean()):.6f}; "
                  f"sample loss ES95={empirical_es(info['terminal_loss'], .95):.6f}; "
...
if __name__ == "__main__":
    ...
    run(arguments.device, arguments.paths)
