...
Paper: Parker-Holder et al., Evolving Curricula with Regret-Based Environment
Design (ICML 2022), https://proceedings.mlr.press/v162/parker-holder22a.html.
Implementation: local finite-task sampler inspired by ACCEL prioritization.
Implementation notes: docs/baseline-methods.md.

ACCEL prioritizes high-regret environments and mutates them. Here only
prioritization is transferred to a declared finite task set. Regret is a gap to
a frozen feasible policy, not an oracle: regret uses task ES, while
pooled_ru_regret uses the current shared Rockafellar--Uryasev threshold.

The target remains pooled terminal ES over the uniform task mixture. Adaptive
sampling uses p(task)/q(task) on the entire RU loss, including the threshold.
"""
