...
Paper: Lee et al., Hyperspherical Normalization for Scalable Deep Reinforcement
Learning (ICML 2025), https://arxiv.org/abs/2502.15280v2.
Upstream: https://github.com/DAVIAN-Robotics/SimbaV2/tree/86899c277cdc697b2b02d827243de1ea93f20a1d
License: Apache-2.0. Implementation notes: docs/baseline-methods.md.

The donor supplies networks, optimizers, updates and normalization equations.
This module supplies holdings coordinates, raw-loss replay and local
...
from hedging_gym.environment.config import RiskConfig
from hedging_gym.environment.finance import observation_fields
...
class SimBaV2Hedger:
    def __init__(self, config, donor_path, *, seed=7, total_updates=1000,
        ...
        from scale_rl.agents.wrappers.normalization import (
    ...
    def load_state_dict(self, state):
        ...
        if (state["source_commit"] != SOURCE_COMMIT
                or state["config"] != asdict(self.config) or state["cfg"] != self.cfg):
