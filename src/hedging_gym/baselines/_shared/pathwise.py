"""Shared pathwise training for direct and no-transaction-band hedgers.

The caller supplies the policy constructor and method identity. Seeding happens
here before construction, preserving the initialization and minibatch streams.
"""
import time
...
def train_pathwise(policy_class, train_bank, *, method, method_label, seed=7, updates=8, batch_size=32,
    ...
    """
    if updates < 1 or batch_size < 1 or len(train_bank.spot) < 1:
        raise ValueError("provide positive training work and a nonempty bank")
    ...
    if progress:
        _report("train_start", method=method, label=method_label, seed=seed,
    ...
    policy = policy_class(config, hidden=hidden)
    ...
    metadata = dict(label="Baseline training", method=method,
                    method_label=method_label, seed=seed, minibatch_seed=seed+100003,
