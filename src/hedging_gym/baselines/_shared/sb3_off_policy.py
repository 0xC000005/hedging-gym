"""Shared financial replay, inference and checkpoint support for SB3-Contrib.

CrossQ and TQC own their upstream algorithm construction in the public method
modules. Replay stores terminal cash loss and applies the current global risk
threshold when sampled. See docs/baseline-methods.md for the objective mapping.
"""
import random
...
def load_off_policy(algorithm_class, directory, env, *, device="cpu"):
    directory = Path(directory)
    ...
    if runtime["algorithm"] != algorithm_class.__name__.lower():
        raise ValueError("checkpoint algorithm does not match the selected method")
    model = algorithm_class.load(directory / "model", env=env,
