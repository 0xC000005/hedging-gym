"""Stochastic AlphaZero adaptation: PUCT search and visit/return fitting.
...
continuation reanalysis are local adaptations, not features of the source paper.

Independent roots share batched pricing/inference; tree traversal remains on
CPU. The default Cartesian action lattice is intended for small hedge sets.
"""
...
class AlphaZeroPolicy(_ConfiguredPolicy):
    ...
    def __init__(self, config, hidden=(64, 64), *, targets=None, grid_points=3):
        ...
        # Refit this scale with critic targets; predictions map back to money.
        ...
        # Keep search exploration and input units fixed during value rescaling.
...
class _State:
    ...
    marks: torch.Tensor
    terminal_cost: float | None = None  # RU-transformed objective in money
    terminal_loss: float | None = None  # Raw cost-inclusive portfolio loss


@dataclass
class _Node:
    """Local tree statistics in cost units: lower values are better."""
    state: object
