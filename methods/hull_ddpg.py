"""Cao/Hull et al. (2021) two-moment DDPG, not 2023 QR-D4PG.

Source: rotmanfinhub/deep-hedging-research@b4d031a185fe2547dd81ad7a67081f6dbe52c5bc.
The common-task adaptation observes the shared schema and controls all legal
holdings. Dense net-portfolio P&L telescopes to the authoritative terminal P&L.
The original mean-minus-1.5-standard-deviation objective is NOT terminal ES.

Explicit source corrections: pointwise nonnegative variance, not K.max(var,0)
(which reduces the replay batch); actual target copies after initialization;
and PER samples all occupied slots, not an exclusive-end off-by-one subset.
Actor BN retains source inference semantics; critic BN and its shared input
layer retain the Keras update convention. Four shuffled 32-row fits per moment
critic and one actor update follow each 128-row replay draw.
"""
...
from .policies import _ConfiguredPolicy, PolicyAction, HOLD, BUY, SELL
...
def train_hull_ddpg(bank, *, episodes=50001, seed=7, device='cpu', collection_batch=16,
    ...
    if progress:
        print(dict(method='Hull2021', config=asdict(config), options=options, device=str(device),
                   transitions=episodes*config.n_steps, updates=episodes*config.n_steps-128), flush=True)
