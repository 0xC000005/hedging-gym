"""ADAPTATION: causal model-priced delta, band and two-sensitivity controls.

These transparent controls hedge the liability's partial spot derivative at
the currently observed variance. The stock-plus-call control additionally
...
from hedging_gym.finance import MarketBank, call_price


def spot_delta(spot, variance, time_index, config, *, chunk_size=1024):
    """Batched model-price partial d(call price)/d(spot), holding variance fixed.
    ...
    roundoff is projected into the European call's r=q=0 delta bounds [0,1].
    """
    if chunk_size < 1:
    ...
    for offset in range(0, spot_values.numel(), chunk_size):
        ...
        with torch.enable_grad():
            ...
            price = call_price(current_spot, variance_values[sl], maturities[sl],
                               config.liability_strike, config)
            delta, = torch.autograd.grad(price.sum(), current_spot)
        result.append(delta.detach())
    return torch.cat(result).reshape(shape).clamp(0.0, 1.0).to(spot.dtype)
...
def delta_hedge_positions(bank: MarketBank, *, band: float = 0.0, deltas=None, chunk_size=1024):
    ...
    lower = (deltas - band).clamp(c.holding_lower[0], c.holding_upper[0])
    upper = (deltas + band).clamp(c.holding_lower[0], c.holding_upper[0])
...
def spot_variance_greeks(spot, variance, time_index, config, *, hedge_index=0, chunk_size=1024):
    ...
    Each contract gets its model-price spot and observed-variance derivatives.
...
def _pair_greeks(spot, variance, time_index, config, *, hedge_index, chunk_size, gamma=False):
    if not 0 <= hedge_index < len(config.hedge_strikes) or chunk_size < 1:
        raise ValueError("select an available hedge call and a positive chunk size")
    ...
    maturities = torch.stack((config.n_steps * config.dt - time,
                              config.hedge_maturities[hedge_index] - time), dim=-1)
    if bool((maturities <= 0).any()):
        raise ValueError("hedge Greeks require decisions strictly before settlement")
    strikes = (config.liability_strike, config.hedge_strikes[hedge_index])
    deltas, dvariances = [], []
    for offset in range(0, s.numel(), chunk_size):
        ...
        with torch.enable_grad():
            ...
            prices = call_price(ss, vv, maturities[sl], strikes, config)
            if gamma:
...
def delta_variance_hedge_positions(bank: MarketBank, hedge_index=0, greeks=None, *, chunk_size=1024):
    """Stock plus a preselected call, matching dS and dv subject to shared caps.

    First q_call=F_v/C_v is projected to its allowed holdings; then
    q_stock=F_S-q_call*C_S is recomputed and projected to the stock bounds.
    Other calls are unused. This is sensitivity matching under caps and daily
    ...
    """
    if not 0 <= hedge_index < len(bank.config.hedge_strikes):
        raise ValueError("select an available hedge call")
    c = bank.config
    if greeks is None:
        times = torch.arange(c.n_steps, device=bank.spot.device)[None, :]
    ...
    call = (dv[..., 0] / dv[..., 1]).clamp(c.holding_lower[instrument], c.holding_upper[instrument])
    stock = (ds[..., 0] - call * ds[..., 1]).clamp(c.holding_lower[0], c.holding_upper[0])
    positions = stock.new_zeros((*stock.shape, c.n_assets))
    positions[..., 0] = stock
    positions[..., instrument] = call
...
def delta_gamma_hedge_positions(bank: MarketBank, hedge_index=0, greeks=None, *, chunk_size=1024):
    """Match liability delta/gamma with stock/call, under the shared bounds.
