"""CEM rollout improvement using fresh conditional scenarios and shared pricing.

The cross-entropy method fits a proposal to elite sampled controls (Rubinstein,
1999). Here it improves the current hedge, then rolls a fixed feedback policy to
expiry. It is a rollout/MPC baseline, not an implementation of full MPPI or
AlphaZero. Learned proposals and optional pathwise refinement are ablations in
the identical planner, not changes to the accounting or market simulator.
"""
...
from hedging_gym.finance import (LedgerState, decode_market_observation,
    mark_state, observation_from_state, transition, trade_step, liquidate)


def _repeat_ledger(ledger, candidates, scenarios):
    # Keep each root's ledger separate; never mutate the evaluator's inputs.
    def repeat(value):
        return value[:, None, None].expand(-1, candidates, scenarios, *value.shape[1:]).reshape(
            -1, *value.shape[1:])
    return LedgerState(*(repeat(getattr(ledger, field.name)) for field in fields(LedgerState)))


@torch.no_grad()
def _conditional_paths(observed, time_index, config, scenarios, generator):
    ...
    return states, marks, liability.to(observed.dtype)
...
class RolloutPlanner:
    ...
    def _score(self, targets, ledger, time_index, config, paths):
        ...
        for offset, date in enumerate(range(time_index, config.n_steps)):
            mid = branch(marks[offset])
            if offset:
                spot, variance = (branch(value) for value in states[offset])
                observed = observation_from_state(spot, variance, date, state, mid, config)
                target = self.continuation(observed, state.positions,
        ...
        loss = liquidate(state, branch(marks[-1]), branch(payoff), config)["terminal_loss"]
    ...
    def __call__(self, observed, ledger, time_index, config):
        ...
        paths = _conditional_paths(observed, time_index, config, self.scenarios, self.generator)
