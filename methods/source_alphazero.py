"""Thin game/network boundary for plan64/minimalHedger_AlphaZero.

Import the donor's MCTS, Trainer and supervised fitting rather than another
search implementation. Finance comes exclusively from hedging_gym.finance.
See docs/source-alphazero.md for the objective and state-aggregation changes.
"""
...
from hedging_gym.instruments import EuropeanOption
...
class SourceHedgingGame:
    ...
    def mark(self, spot, variance, date, integral=None):
        """Scalar QuantLib pricing; batched evaluation retains the tensor pricer.

        Search needs hedge marks, but not unused intermediate liability prices.
        Both backends use identical models/contracts and are checked for parity.
        """
        c = self.config
        def price(option):
            remaining = (round(option.maturity*c.time_grid.days_per_year)
                         - min(date, c.n_steps)*c.time_grid.step_days) / c.time_grid.days_per_year
            if not isinstance(option, EuropeanOption):
                return float(option.mark(spot, variance, remaining, c.market, integrated_variance=integral)[0])
            if float(variance[0]) == 0.:
                # QuantLib HestonModel requires strictly positive v0. QE can
                # reach zero; preserve that state, using our tensor formula
                # rather than silently flooring variance to fit its interface.
                return float(finance.option_price(spot, variance, remaining,
                    option.strike, c.market, kind=option.kind)[0])
            return finance.quantlib_option_price(float(spot[0]), float(variance[0]),
                remaining, option.strike, c.market,
                days_per_year=c.time_grid.days_per_year, kind=option.kind)
        marks = spot.new_tensor([[float(spot[0]), *(price(option) for option in c.portfolio.hedges)]])
        liability = (spot.new_tensor([c.portfolio.liability_quantity*price(c.portfolio.liability)])
                     if date >= c.n_steps else None)
