"""On-policy categorical terminal-risk updates with frozen continuous sizing.

The policy parametrization comes from ``methods.hybrid`` and its pinned HPO
source attribution. This experimental adapter compares an all-mode conditional
expectation with a sampled score-function update. It is not the full HPO mixed
gradient/PPO algorithm, and makes no novelty or policy-improvement guarantee.
Market suffixes are training data; deployed policies receive current state only.
See docs/counterfactual-update.md for sources, derivation context and commands.
"""
...
from hedging_gym.finance import LedgerState, bank_subset, bank_to
...
def train_counterfactual(source_policy, train_bank, *, algorithm, zeta, seed=7,
    ...
    return policy, dict(method=algorithm, seed=seed, device=str(device), options=options,
        ...
        training_paths=len(bank.spot), source="HybridPolicy from methods.hybrid; categorical-only adaptation",
