"""Official AMAGO interfaces for unchanged common-ledger episodes.

AMAGO learns expected discounted return. With gamma=1 and a fixed global RU
threshold, terminal negative RU rewards optimize that exact expected RU
criterion. This is *not* joint ES minimization until threshold optimization is
added. Keeping the threshold fixed also keeps old replay rewards valid.

Concatenating independent books is a training sequence, not a new market model.
All declared market parameters remain in the original observation. No regime
label or future path is added. Native agent code lives outside this repository.
"""
