"""GEPS context-controlled low-rank layers on the common hedging policy.

Implements equations (4)--(5) of Kassaï Koupaï et al., NeurIPS 2024:
https://arxiv.org/abs/2410.23889. Author-layer qualification is against
``itsakk/geps`` at ``GEPS_SOURCE_COMMIT``, not a PDE experiment reproduction.
Training jointly fits shared matrices and source contexts; there is no inner
meta-gradient loop. The inherited adapter then fits only a new task context.
"""
