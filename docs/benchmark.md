...
Serialized configurations record the selected scheme. For backward
compatibility, `config_from_dict` loads a Heston or Bates market dictionary
without `scheme` as `"qe"`, preserving the historical sampler. To migrate a
saved configuration deliberately, set its market `scheme` to `"qe_m"`.
The baseline runner accepts `--scheme qe` for legacy runs; new Heston/Bates
runs default to QE-M. Compare methods on the same scheme, not across old and new
score tables.
