...
paper presets or QE-M defaults. Historical replay uses the archived source and
artifacts. Maintained runners use the current configuration and require matching
current checkpoints; no backward-compatibility layer is maintained.
...
reproduce their paper tables. See [methods and sources](baselines.md).
