...
```bash
python -m benchmarks.compare_fast_adaptation banks --output /path/to/run --device cuda
```

Then run each method (`adh`, `geps`, `belief`) with policy seeds 7, 17 and 29:

```bash
python -m benchmarks.compare_fast_adaptation compare --output /path/to/run --method adh --seed 7 --threads 1
```
...
are in `benchmarks/compare_fast_adaptation.py`.

```bash
python -m benchmarks.summarize_fast_adaptation /path/to/run
```
...
```bash
python -m benchmarks.compare_update_capacity banks --output /path/to/capacity-run --device cuda
python -m benchmarks.compare_update_capacity compare --output /path/to/capacity-run --checkpoints /path/to/original-run --seed 7
```

Repeat the second command for seeds 17 and 29, then summarize:

```bash
python -m benchmarks.summarize_update_capacity /path/to/capacity-run
```
...
`src/hedging_gym/extensions/meta_pretraining.py` adapts the first-order version of
...
```bash
python -m benchmarks.compare_adapt_aware banks --output /path/to/new-run --device cuda
python -m benchmarks.compare_adapt_aware train --output /path/to/new-run --source /path/to/original-run --family meta --seed 7
python -m benchmarks.compare_adapt_aware develop --output /path/to/new-run --source /path/to/original-run --development /path/to/capacity-run --family meta --seed 7
```
...
```bash
python -m benchmarks.compare_adapt_aware select --output /path/to/new-run --development /path/to/capacity-run
python -m benchmarks.compare_adapt_aware test --output /path/to/new-run --source /path/to/original-run --family meta --seed 7
```
...
```bash
python -m benchmarks.compare_adapt_aware diagnose --output /path/to/new-run --source /path/to/original-run --development /path/to/capacity-run --family meta --seed 7
```

Reconstruct the results and audit saved losses and representative cash paths:

```bash
python -m benchmarks.summarize_adapt_aware --output /path/to/new-run --development /path/to/capacity-run
```

## Attribute an improvement before adding another method

`benchmarks.attribute_pretraining` compares ordinary source continuation with
...
```bash
python -m benchmarks.attribute_pretraining run --source /path/to/original-run --development /path/to/capacity-run --output /path/to/attribution --loss ru --batch 256 --seed 7
```
...
```bash
python -m benchmarks.attribute_pretraining summarize --development /path/to/capacity-run --output /path/to/attribution
```
