...
|---|---|
| Bühler, Gonon, Teichmann and Wood, [Deep Hedging](https://arxiv.org/abs/1802.03042v1) | Learning constrained hedging strategies under transaction costs and a terminal risk objective. Our default benchmark uses a tradable European call; the original paper uses a variance-swap hedge, available through the paper configuration. |
| Godin, [Is Deep Hedging Reinforcement Learning?](https://arxiv.org/abs/2607.13353) | Discusses Deep Hedging as actor-only pathwise-gradient policy learning within reinforcement learning; actor-critic and search methods offer different learning and decision mechanisms. |
| Buehler, Murray and Wood, [Deep Bellman Hedging](https://arxiv.org/abs/2207.00932v4) | Actor-critic value iteration for hedging under a monetary utility, with existence and uniqueness of the Bellman fixed point. [Our implementation](../src/hedging_gym/baselines/deep_bellman_hedging.py) samples simulated books instead of tabulated history and keeps the reward inside the utility as in the paper's definition. |
| Murray, Wood, Buehler, Wiese and Pakkanen, [Deep Hedging: Continuous Reinforcement Learning for Hedging of General Portfolios across Multiple Risk Aversions](https://arxiv.org/abs/2207.07467), ICAIF 2022 | Numerical companion of Deep Bellman Hedging: an entropic-utility actor-critic on a Heston market with a target critic, a delta skip connection, a book-value critic residual and on-policy episodes. `dbh` implements the theory paper's scheme without these additions; see the [implementation guide](baseline-methods.md#training-objectives-are-not-interchangeable). |
