...
| Cao/Hull 2023 QR-D4PG | Quantile critic, n-step replay targets and deterministic actor gradients through conditional risk | The original TensorFlow learner receives the shared ledger through a small environment bridge. Joint holdings replace one gamma/vega-normalized option action plus automatic delta hedging. Reward units are explicitly scaled; native conditional CVaR is retained. |
| EX-DRL | QR-D4PG with a fitted generalized Pareto tail used in targets and actor optimization | The author learner and selected native conditional-risk objective are retained. Batched GPD likelihood and streamed quantile-loss calculations change execution, not the intended loss. Source sampling quirks are recorded rather than silently repaired. |
| Adaptive Deep Hedging | Shared network and source-task embeddings; adapt a new embedding with shared weights frozen | Paper-mechanism implementation with common observations, stock/call book and ES rather than the paper's frictionless squared-error task. Full-network fine-tuning is a separate control. |
| Neural MCTS / AlphaZero | Network-guided search, visit-distribution policy targets and realized-return value targets | The public Szehr release is qualified separately. Common Heston/action-grid/risk-objective changes must be identified; the existing custom ES/reanalysis adapter is not automatically a faithful author-code baseline. |
...
| CrossQ (qualification pending) | SB3-Contrib's batch-normalized SAC-style learner without target networks | Continuous stock/call holdings and a terminal global-risk reward. Replay rewards must use the current training-only ES threshold, not the threshold stored when a path was collected. |
| TQC (qualification pending) | SB3-Contrib's truncated ensemble of continuous quantile critics | Same financial interface and global-risk reward as CrossQ. Quantile truncation addresses critic overestimation; the reward mapping, not truncation, supplies the ES objective. |
| SimBaV2 (qualification pending) | Official JAX learner: hyperspherical normalization and SAC-style distributional value learning | Thin observation/action/reward bridge, with the same financial ledger and current-threshold replay treatment. No new market model or discretized action grid. |
...
The earlier PyTorch `hull_rl` and `exdrl` global-risk ports remain diagnostic
adaptations. Native author-code success and the successful original-learner
Heston transfer do not retroactively qualify those different ports.
